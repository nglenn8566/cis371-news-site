...
<div>
<b-container>
  <h2 class="text-center white">Business Headlines</h2>
    ...
    recentNews(){
      ...
      fetch(req)
       ...
       firebase.database().ref().child("timeUpdated").limitToFirst(1).once('value',function(snapshot){
          lastTopUpdate = moment(snapshot.val().businessArticles)
          timeDif = currentMoment.diff(lastTopUpdate,'hours',true)
          if(timeDif >= 6){
            for(var item of jsonData.articles){
              firebase.database().ref().child("businessArticles").push().set(item)
              var updates ={};
              updates['/timeUpdated/businessArticles'] = moment().utc().format()
